...
Dac::Dac(Spi *spi, uint8_t cs_port, uint8_t cs_pin)
    ...
    uint8_t config_byte =
        GetConfigByte(kDacChannelA, kBuffered, kGain1, kActive);
...
uint8_t Dac::GetConfigByte(DacChannel channel, VrefBuffer buffered,
                           DacVrefGain gain, DacShutdown active) {
